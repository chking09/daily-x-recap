...
The Daily Market Recap is a simple data pipeline that collects financial data from two APIs, processes it with Claude AI, and publishes the result to X (Twitter). The system follows a linear workflow: **Collect → Process → Publish**.
...
### Core Components
1. **Data Collector**: Interfaces with Google Custom Search API and Alpha Vantage API
...
**Purpose**: Gather financial news and stock data from external APIs
...
**Dependencies**:
- Google Custom Search API client (`googleapiclient`)
- Alpha Vantage Python client (`alpha_vantage`)

**Key Implementation Details**:
- Google Custom Search: Use `customsearch.cse().list()` with financial news queries
- Alpha Vantage: Use `TimeSeries.get_quote_endpoint()` for current stock data
- Filter stocks by market cap > $2 billion
...
- Alpha Vantage returns valid stock data with market cap filtering
...
├── data_collector.py       # Google Search + Alpha Vantage
